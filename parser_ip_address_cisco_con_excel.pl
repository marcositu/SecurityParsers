#!/usr/bin/perl
#Obtengo todas las IPs de los equipos cisco, guardo todo en excel, columna y debajo la IP correspondiente
# 27/10/2012
...
my $workbook  = Excel::Writer::XLSX->new("$fecha-cisco.xlsx");
...
while (my $file = readdir(DIR)) {
    ...
    while (my $linea = <INPUT>) {
			if ($linea =~ m/^.*RANCID.*\scisco/ ) {                       # si hay una 'cisco'
            my $fila = 0;
            ...
            while (<INPUT>) {                               # ahora buscaremos IP
                if (m/^\sip\saddress\s([0-9|\.].*)\s2/) {
                    $worksheet->write($fila++, $columna, $1);
                }
            }
 
            $columna++;                # apuntamos a la siguiente columna
            last;                      # no hace falta seguir buscando más 'cisco'. Salimos del while
        }
    }
